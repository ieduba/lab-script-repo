...
```{r}
genecounts <- read.table(file="~/linker-histone/RNA-seq/willcockson/all-genefeaturecounts.txt", header = TRUE)
colnames(genecounts) <- c("GeneID","Chr","Start","End","Strand","Length","cTKO1","cTKO2","cTKO3","cTKO4","cTKO5","cTKO6","WT1","WT2","WT3","WT4","WT5","WT6")
counts <- dplyr::select(genecounts,cTKO1,cTKO2,cTKO3,cTKO4,cTKO5,cTKO6,WT1,WT2,WT3,WT4,WT5,WT6)
rownames(counts) <- gsub('\\.[0-9]*$', '',genecounts$GeneID)

colData <- as.data.frame(x = c("cTKO", "cTKO", "cTKO", "cTKO", "cTKO", "cTKO", "WT", "WT", "WT", "WT", "WT", "WT"))
colnames(colData) <- c("genotype")
colData$genotype <- as.factor(colData$genotype)
colData$genotype <- factor(colData$genotype,levels = c("WT", "cTKO"))
...
de.dds <- DESeq(dds)
write.csv(assay(de.dds), "/lustre/fs4/risc_lab/scratch/iduba/linker-histone/RNA-seq/willcockson/DESeq2_results/dds_raw_counts_matrix-noctrl.csv")
plotDispEsts(de.dds)

resLFC <- lfcShrink(de.dds, coef="genotype_cTKO_vs_WT", type="normal")
resLFC.noNA <- na.omit(resLFC)
ggplotMA(resLFC.noNA, 1:20, "WTvscTKO (LFC)")
...
write.csv(resLFC.df, "/lustre/fs4/risc_lab/scratch/iduba/linker-histone/RNA-seq/willcockson/DESeq2_results/WTvscTKO-noctrl.csv")

WTvslowup <- subset(subset(resLFC, padj < 0.05), log2FoldChange > 1)
WTvslowdown <- subset(subset(resLFC, padj < 0.05), log2FoldChange < -1)
write.csv(WTvslowup, "/lustre/fs4/risc_lab/scratch/iduba/linker-histone/RNA-seq/willcockson/DESeq2_results/WTvscTKOup-noctrl.csv")
write.csv(WTvslowdown, "/lustre/fs4/risc_lab/scratch/iduba/linker-histone/RNA-seq/willcockson/DESeq2_results/WTvscTKOdown-noctrl.csv")
```
