...
#			rna,dna=trimrnadna(prna,pdna)
			...
			dna=reverse_complement(backwardsdna)
			rna=reverse_complement(backwardsrna)
...
def fuzz_align_once(s_seq,l_seq,mismatch):
	for i, base in enumerate(l_seq):  # loop through equal size windows
		...
		dist = levenshtein_distance(l_subset, s_seq)
...
def bridgepos_multicheck(bridge,line):
	...
	fuzz=fuzz_align_once(bridge,line,mismatch)
	#if you didnt find it, try the revcomp of the bridge
	if not(fuzz):
		fuzz_rc=fuzz_align_once(reverse_complement(bridge),line,mismatch)
		if not(fuzz_rc):
			...
			fuzz_rc_2=fuzz_align_once(reverse_complement(bridge),restofline,mismatch)
			if not(fuzz_rc_2):
		...
		fuzz_fw=fuzz_align_once(bridge,restofline,mismatch)
		#and the rc
		fuzz_rc=fuzz_align_once(reverse_complement(bridge),restofline,mismatch)
