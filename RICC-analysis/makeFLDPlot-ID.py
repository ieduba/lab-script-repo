...
# 
# Irene Duba edits 2021: expects input bed file sorted by GC content. output is GC normalized FLFE matrix and matrix of averages across GC content
...
opts.add_option("-g", default="100", help="number of GC quantiles to use, default=100")
...
def sub_Mat(start):
    ...
    # get average across chunk rows
    matavg = np.average(mat,0)
    # divide the matrix row-wise by the average, avoiding dividing by zero
    with np.errstate(invalid='ignore', divide='ignore'):
        FLFE = np.where(matavg > 0, np.divide(mat, matavg[None,:]), 0)
    return mat, matavg, FLFE
...
# split bedfile into chunks ##ID EDIT## changing this to # GC chunks option (defailt 100) rather than -c thread option
maxi=len(p1_ints)
Nchunks=int(options.g)
chunksize=int(maxi/Nchunks)
...
if __name__ == "__main__":
    pool = Pool(processes=int(options.g))
    sub_mats, cohort_avgs, subFLFEs = zip(*pool.map(sub_Mat, starts,1))
...
for i in range(len(starts)):
    ...
    mat[chunkstart:chunkend,:] = subFLFEs[i]

#assemble matrix of chunk averages
avmat = np.zeros([len(cohort_avgs),cols])
for i in range(len(cohort_avgs)):
    avmat[i,:] = cohort_avgs[i]
print(avmat.shape)
...
#save average matrix
np.savetxt(options.o+'.avg',avmat,delimiter='\t',fmt='%s')
