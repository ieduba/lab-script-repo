...
# Jan 20, 2021 - V4 changed asn_mat function to add weights from each fragment to a single row vector rather than holding it all as a huge matrix in memory. The row vectors get concatenated and columns summed to combine the work from each core. Currently -v does nothing and matrix is not output using the existing code. 
...
if __name__ == "__main__":
    ...
    #concatenate row vectors by stacking them into matrix
    sub_mats_con = np.concatenate((sub_mats), axis = 0)
    #sum each column to create single vector FLD
    sub_mats_sumcol = sub_mats_con.sum(axis = 0)
    #save FLD vector as CSV
    np.savetxt("temp_sub_mats_sumcol.csv", sub_mats_sumcol, delimiter =",")
...
if options.u == True:
    if options.n == True:
        ...
        np.savetxt(options.o,np.hstack((intnames,sub_mats_sumcol)),delimiter='\t',fmt='%s')
    else:
        if np.size(p1_ints,1) >= 4:
            intnames=np.reshape(p1_ints[:,3],(rows,1))
            np.savetxt(options.o+'.intnames',intnames,delimiter='\t',fmt='%s')
        np.savetxt(options.o,sub_mats_sumcol,delimiter='\t',fmt='%s')
else:
    np.save(options.o,sub_mats_sumcol)
...
#yran=min(500,rows)
if options.v == True:
    plt.plot(sub_mats_sumcol,'k-')
    plt.plot(np.convolve(sub_mats_sumcol,np.ones(int(options.window)),'same')/int(options.window),'r')
    plt.xlabel('Fragment length (bp)')
    plt.ylabel('Counts over all regions')
else:
    plt.imshow(sub_mats_sumcol,origin='lower',aspect='equal')
    plt.xlabel('Fragment length (bp)')
    plt.ylabel('Region')
